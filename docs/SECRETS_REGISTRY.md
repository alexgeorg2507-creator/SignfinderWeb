...
| DeepSeek API key | `deepseek-api-key` — ✅ в env (`secretKeyRef`, подтверждено `gcloud run services describe` на ready-ревизии `signfinder-api-00059-sb5`, 2026-07-25) | `deepseek-key` — ✅ в env | ✅ Закрыто на test 2026-07-25 (было `❌ НЕТ в env` в более ранней версии этой таблицы — устарело относительно живого состояния). См. `TASK_bugfix_llm_step3.md`: текущий баг Шага 3 на test не связан с этим секретом — реальная причина в `signfinder-core` (deprecated модель `deepseek-chat`) |
| Внутренний API_KEY | `api-key` — ✅ в env (`secretKeyRef`, подтверждено вместе с DeepSeek, 2026-07-25) | `api-key` — ✅ в env | ✅ Закрыто на test. Ранее подозревался как `❌ НЕТ в env` — устарело |
...
| Firebase Web `apiKey` (`SignfinderLand/app/index.html`, блок `FIREBASE_CONFIG`) | свой `apiKey` для `signfinder-cab-test` | свой `apiKey` для `signfinder-prod` | ✅ Не секрет by design — публичный идентификатор для `initializeApp()`, обязан лежать в клиентском JS. Граница безопасности — Authorized domains (Firebase Auth) + Security Rules, не секретность ключа. `gitleaks` (правило `gcp-api-key`) ловит любую строку формата `AIza...` по regex, не по чувствительности — срабатывание на эти два ключа игнорировать, НЕ ротировать. См. инцидент 2026-07-24 в `RUNBOOK_TECH_SECURITY.md` |
| **`tg-feedback-bot-token`** *(v2.0.0)* | будет создан в E6 | будет создан в E6 | ⏳ Токен Telegram-бота для доставки опросника «Что улучшить?» в личный чат владельца. Создаётся через @BotFather → `/newbot` → сохранить `HTTP API token`. **Не публиковать никогда** — с ним можно писать от лица бота |
| **`tg-feedback-chat-id`** *(v2.0.0)* | будет создан в E6 | будет создан в E6 | ⏳ Chat ID владельца (числовой). Получается один раз: написать `/start` боту → `curl https://api.telegram.org/bot{TOKEN}/getUpdates` → взять `result[0].message.chat.id`. Не секрет как таковой (это просто число), но лежит рядом с токеном для удобства |

**Что v2.0.0 явно НЕ добавляет в реестр:**
- Postmark / SES / SendGrid API keys — SignFinder не отправляет email
  никому (см. ADR-010). Если в какой-то момент в будущем эти секреты
  всё-таки появятся — это будет означать отход от ADR-010 и требует
  нового ADR.
- SMTP username/password — по той же причине.
...
| `tg-feedback-bot-token` *(v2.0.0)* | При подозрении на утечку — через @BotFather → `/revoke` → новый токен, обновить в Secret Manager. Иначе — не ротировать |
...
1. Немедленно инвалидировать старый ключ у провайдера (DeepSeek console / GCP / @BotFather)
...
**Специально для Telegram-бота опросника:** утечка токена = злоумышленник
может писать от имени бота владельцу. Это не критично (владелец сразу
поймёт что что-то не так и разберётся), но токен всё равно ротировать
через @BotFather. Chat ID не является секретом.
