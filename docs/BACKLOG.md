...
- [ ] Privacy Policy / ToS / Refund Policy / Cookie Policy — на проде реальные
  email и подписи людей, юридического документа нет (флаг из `00_INDEX.md`,
  не закрыт). **С v2.0.0 Deal Cycle актуальность растёт:** финальный PDF
  со сделки содержит IP обеих сторон. Одновременно упрощается —
  SignFinder не оператор рассылок (не шлёт email), так что часть
  требований к Privacy Policy отпадает. Плюс новый пункт: явно прописать
  7-дневный срок жизни ссылки после signed (см. THREAT_MODEL_DEAL_CYCLE.md §3.G).
  **Кроме того — блокер биллинга (ADR-011).** Без ToS/Privacy/Refund
  подключить Keepz нельзя. Генераторы (Termly/iubenda/Termageddon)
  закрывают все четыре документа за $10-30/месяц.
...
- [ ] **Биллинг (платные подписки)** — архитектурное направление выбрано
  в ADR-011 (Keepz + Credo Bank + ИП Georgia Small Business Status). Тарифы:
  $5/мес (карта recurring / крипта one-time вручную), $50/мес (то же),
  $500 одноразовая. Техническая включаемость — только после первых
  `limit_reached` в PostHog **и** 3-5 разговоров с платящими пользователями
  Deal Cycle (см. `RUNBOOK_MARKETING.md`, `DEAL_CYCLE_SPEC.md`). Пред-реквизиты
  к включению:
  - [ ] **Пред-онбординг Keepz — закрыть 5 вопросов из ADR-011**
    (список монет CITYPAY включая USDT TRC-20, итоговая комиссия,
    активация saveCard/subscriptionPlan/custom callback, recurring крипта,
    география карт) — до подписания договора
  - [ ] **ИП Georgia Small Business Status** оформлен, бизнес-счёт
    Credo Bank открыт
  - [ ] **ToS + Privacy Policy + Refund Policy + Cookie Policy опубликованы**
    (блокер запуска платежей, см. P1 Governance ниже)
  - [ ] **Webhook-обработчик** в `signfinder-api` с гибридной дешифровкой
    AES-256-CBC + RSA + таблица `subscriptions` в БД (FK на `users`),
    связь с `usage_counters`. Новые секреты в `SECRETS_REGISTRY.md`
  - [ ] **UI прайс-страницы** + интеграция `POST /api/integrator/order`
    через signfinder-api-прокси (ключи Keepz на сервере, не в браузере)
