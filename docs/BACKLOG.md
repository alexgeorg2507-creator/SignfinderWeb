...
- [ ] Privacy Policy / ToS — на проде реальные email и подписи людей,
  ...
  **В работе (2026-07-31):** юрисдикция решена владельцем — хранение как
  есть (ЕС), ФЗ-63 только как основание ПЭП. Текст всех трёх документов
  (оферта/конфиденциальность/cookies) написан, см. `TASK_fix20.md`.
  **Припарковано (2026-08-06, `TASK_fix22.md`):** документы и гейт
  принятия условий (`TASK_fix21.md`) готовы технически, но публикация
  снята с test — плейсхолдер `[ИМЯ/РЕГИСТРАЦИЯ ВЛАДЕЛЬЦА]` не заполнен,
  легального ИП в Грузии ещё нет, подписываться под текстом некому.
  Условие возврата: 1) оформить ИП, 2) заполнить плейсхолдер в
  `terms.html`/`privacy.html`, 3) решить нужен ли review юриста перед
  реальным запуском — только тогда возвращать fix20 §1/§2 в обратную
  сторону (файлы и гейт уже готовы, ~20 минут работы, не переписывать
  заново). Прод не трогали — там этой публикации никогда не было.
...
- [ ] **Юрисдикционное основание ПЭП для не-РФ контрагентов** (всплыло при
  планировании EN-версии, `TASK_i18n_en.md` §0): англоязычный
  контрагент может быть в другой юрисдикции, ФЗ-63 как основа может не
  применяться. Блокирует как EN-версию лег. документов (§6 там же —
  только каркас, без контента), так и `_append_legal_block` (E7) в части
  юр. текста в PDF для EN-сделок
- [ ] **EN-лендинг — маркетинговая полировка** после функционального перевода
  из `TASK_i18n_en.md` §5 — позиционирование/тон текста, не техническая задача
- [ ] **Тоггл языка на самой странице подписания** для контрагента — сейчас
  только наследование языка от сделки (`TASK_i18n_en.md` §4), без локального
  переключателя для самого контрагента
- [ ] **Частичная анонимизация реквизитов перед LLM-вызовом** (обсуждалось
  с владельцем 2026-07-31, при обсуждении юрисдикции): regex-детект и
  замена на плейсхолдер паспортных данных/ИНН/счёта/телефона/адреса
  перед отправкой в LLM. Имена сторон не трогать — `extraction.py` и новый
  `TASK_fix19.md` (имя контрагента) функционально требуют реальные имена
  на входе. Не блокер, но потенциально сильный дифференциатор для
  юротделов/агентств (тот же сегмент что в fix17 — локальная установка)
- [ ] **PostHog на `/sign/{token}` стартует до согласия** (найдено при написании
  cookie-политики, `TASK_fix20.md` §5): `posthog.init(..., capture_pageview: true)`
  срабатывает при загрузке страницы, до того как контрагент увидит
  чекбокс согласия на ПЭП. Cookie-политика теперь описывает это честно, но
  саму последовательность не меняет. Полноценный cookie-consent gate
  (блокировать PostHog до явного согласия) — отдельная задача, не в
  fix20
